...
def fft2c(data: torch.Tensor) -> torch.Tensor:
    ...
    data = torch.view_as_real(torch.fft.fftn(torch.view_as_complex(data), dim=(-2, -1), norm="ortho"))
...
def ifft2c(data: torch.Tensor) -> torch.Tensor:
    ...
    data = ifftshift(data, dim=[-3, -2])
    data = torch.view_as_real(torch.fft.ifftn(torch.view_as_complex(data), dim=(-2, -1), norm="ortho"))
    data = fftshift(data, dim=[-3, -2])
